...
export function EditPostForm({ post, onClose, onSuccess }: EditPostFormProps) {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      ...
      photos: post.photos,
      dryMixture: post.dryMixture || '',
      urineSystem: post.urineSystem || 'not_separated',
      hasCollectionService: !!post.collectionService?.name || !!post.collectionService?.website,
      collectionService: post.collectionService || {
        name: '',
        website: ''
      },
      showContactInfo: !!post.contactInfo?.name || !!post.contactInfo?.phone || 
                     !!post.contactInfo?.email || !!post.contactInfo?.website,
  ...
  async function onSubmit(data: FormData) {
    ...
    try {
      ...
      await updatePost(post.id, postData, firestore);
      onSuccess();
    } catch (error) {
